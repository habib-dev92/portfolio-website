  ...
  return (
    <motion.div
      ...
      className={`group relative overflow-hidden rounded-2xl border border-border bg-card hover:shadow-2xl transition-all duration-500 ${
        isAccent ? "hover:shadow-accent/10 hover:border-accent/30" : "hover:shadow-primary/10 hover:border-primary/30"
      }`}
    >
      <div className={`absolute top-0 left-0 w-1 h-0 ${isAccent ? "bg-accent" : "bg-primary"} group-hover:h-full transition-all duration-500 ease-out z-30`} />
      <div className={`aspect-video bg-gradient-to-br ${project.gradient} flex items-center justify-center overflow-hidden relative`}>
        ...
        <div className={`absolute inset-0 ${isAccent ? "bg-[radial-gradient(circle_at_50%_50%,rgba(6,182,212,0.12),transparent_70%)]" : "bg-[radial-gradient(circle_at_50%_50%,rgba(37,99,235,0.12),transparent_70%)]"} opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10`} />
      </div>
      <div className="p-5 sm:p-6 relative">
        ...
        <div className="flex items-center justify-between mb-2">
          <span className={`text-xs ${isAccent ? "text-accent" : "text-primary"} font-semibold tracking-wide flex items-center gap-1.5`}>
            <span className={`w-1.5 h-1.5 rounded-full ${isAccent ? "bg-accent" : "bg-primary"} animate-breathe`} />
            {project.category}
          </span>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {project.tech.map((t) => (
            <span key={t} className={`text-xs px-2.5 py-1 rounded-full bg-background text-muted border border-border ${
              isAccent ? "group-hover:border-accent/20 group-hover:bg-accent/[0.03]" : "group-hover:border-primary/20 group-hover:bg-primary/[0.03]"
        ...
        {project.repo ? (
          ...
          >
            ...
            <span className="group-hover/link:underline underline-offset-2">View Source Code</span>
          </a>
        ) : (
